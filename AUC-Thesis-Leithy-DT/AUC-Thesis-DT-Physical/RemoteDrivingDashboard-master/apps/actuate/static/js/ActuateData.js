...
//       clicked_id == 1 ? 0 : 1
  ...
  $(document).keydown(function(e) {
    ...
    //     socket.emit('new_data', JSON.stringify(actuate_dict));
      ...
      $('.down').css('transform', 'translate(0, 2px)');
      socket.emit('new_data', 's');
    } else if (e.which == 77) { // 'm' key (mode)
      $('.m').addClass('pressed');       // Add pressed effect
      $('.mtext').text('MODE');          // Show "MODE" label
      $('.m').css('transform', 'translate(0, 2px)'); // Slight push-d
      console.log("Stop triggered");
      socket.emit('new_data', 'm');
    } else if (e.which == 70) { // 'f' key (Stop)
      $('.f').addClass('pressed');       // Add pressed effect
      $('.ftext').text('FREEZE');          // Show "MODE" label
      $('.f').css('transform', 'translate(0, 2px)'); // Slight push-d
      console.log("Stop triggered");
      socket.emit('new_data', 'f');
    }
    else if (e.which == 78) { // 'n' key (Stop)
      $('.n').addClass('pressed');       // Add pressed effect
      $('.ntext').text('change robot');          // Show "MODE" label
      $('.n').css('transform', 'translate(0, 2px)'); // Slight push-d
      socket.emit('new_data', 'n');
    }
    });
    ...
    if (e.which==37 ||e.which==65 ) {
      ...
      $('.m').removeClass('pressed');       // Add pressed effect
      $('.mtext').text('');          // Show "MODE" label
      $('.m').css('transform', 'translate(0, 0)'); // Slight push-d
      //console.log("Stop triggered");
    }
    else if (e.which == 70) { // 'f' key (Stop)
      $('.f').removeClass('pressed');       // Add pressed effect
      $('.ftext').text('');          // Show "MODE" label
      $('.f').css('transform', 'translate(0, 0)'); // Slight push-d
      console.log("Stop triggered");
    }
    else if (e.which == 78) { // 'n' key (Stop)
      $('.n').removeClass('pressed');       // Add pressed effect
      $('.ntext').text('');          // Show "MODE" label
      $('.n').css('transform', 'translate(0, 0)'); // Slight push-d
    }
    });