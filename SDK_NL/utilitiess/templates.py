...
class FileWriterr:
    ...
    def callback_function(self, subscribers, publishers):
        for sub in subscribers:
            name = subscribers[sub]["name"]
            sub_name = name.replace("/", "_")
            pub_name = publishers[sub]["name"].replace("/", "_")
            callback = sub_name + "_callback"
            self.file.write(
                f"""def {callback}(data):
    print('IMPLEMENT CALLBACK {callback}')\n"""
            )
            self.file.write(
                f"""    {pub_name}_pub.publish(data)\n"""
            )
            self.file.write("\n")

    def mw_callback_function(self, subscribers):
        for sub in subscribers:
            name = subscribers[sub]["name"]
            name = name.replace("/", "_")
    ...
    def write_publishers(self, publishers):
        for pub in publishers:
            ...
            self.file.write(
                f"""
{new_name}_pub = rospy.Publisher('/{name}', {type.split("/")[-1]}, queue_size=10)\n"""
            )
        self.file.write("\n")
    ...
    def write_subscribers(self, subscribers):
        for sub in subscribers:
            ...
            callback = new_name + "_callback"
            self.file.write(
                f"""
{new_name}_sub = rospy.Subscriber('/{name}',{type.split("/")[-1]},{callback})
            """
            )
        self.file.write("\n")
